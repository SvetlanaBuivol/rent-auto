...
import {
  ...
  RentCar,
  TextBtnWrapper,
} from './ModalAdvertContent.styled';
...
const ModalAdvertContent = ({ car }) => {
  ...
  return (
    <>
      ...
      <FeaturesText>{`${car.functionalities.join(
        ' | '
      )} | ${car.accessories.join(' | ')}`}</FeaturesText>
      ...
      <TextBtnWrapper>
        ...
        <ModalTextBtn>
          Mileage:<span>{car.mileage && car.mileage / 1000}</span>
        </ModalTextBtn>
        <ModalTextBtn>
          Price:<span>{car.rentalPrice && car.rentalPrice.slice(1)}$</span>
        </ModalTextBtn>
      </TextBtnWrapper>
